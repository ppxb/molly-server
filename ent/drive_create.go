...
func (_c *DriveCreate) check() error {
	if _, ok := _c.mutation.DriveID(); !ok {
		return &ValidationError{Name: "drive_id", err: errors.New(`ent: missing required field "Drive.drive_id"`)}
	}
	if v, ok := _c.mutation.DriveID(); ok {
		if err := drive.DriveIDValidator(v); err != nil {
			return &ValidationError{Name: "drive_id", err: fmt.Errorf(`ent: validator failed for field "Drive.drive_id": %w`, err)}
		}
	}
	if _, ok := _c.mutation.Name(); !ok {
	...
	if _, ok := _c.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "created_at", err: errors.New(`ent: missing required field "Drive.created_at"`)}
	}
	if _, ok := _c.mutation.UpdatedAt(); !ok {
		return &ValidationError{Name: "updated_at", err: errors.New(`ent: missing required field "Drive.updated_at"`)}
	}
	return nil
}
