...
func (_c *S3BucketCreate) check() error {
	...
	if _, ok := _c.mutation.UserID(); !ok {
		return &ValidationError{Name: "user_id", err: errors.New(`ent: missing required field "S3Bucket.user_id"`)}
	}
	if v, ok := _c.mutation.UserID(); ok {
		if err := s3bucket.UserIDValidator(v); err != nil {
			return &ValidationError{Name: "user_id", err: fmt.Errorf(`ent: validator failed for field "S3Bucket.user_id": %w`, err)}
		}
	}
	if _, ok := _c.mutation.Region(); !ok {
	...
	if v, ok := _c.mutation.Region(); ok {
		if err := s3bucket.RegionValidator(v); err != nil {
			return &ValidationError{Name: "region", err: fmt.Errorf(`ent: validator failed for field "S3Bucket.region": %w`, err)}
		}
	}
	if _, ok := _c.mutation.VirtualPathID(); !ok {
		return &ValidationError{Name: "virtual_path_id", err: errors.New(`ent: missing required field "S3Bucket.virtual_path_id"`)}
	}
	if _, ok := _c.mutation.Versioning(); !ok {
		return &ValidationError{Name: "versioning", err: errors.New(`ent: missing required field "S3Bucket.versioning"`)}
	}
	if v, ok := _c.mutation.Versioning(); ok {
		if err := s3bucket.VersioningValidator(v); err != nil {
			return &ValidationError{Name: "versioning", err: fmt.Errorf(`ent: validator failed for field "S3Bucket.versioning": %w`, err)}
		}
	}
	if _, ok := _c.mutation.CreatedAt(); !ok {
