...
// QueryPowers chains the current query on the "powers" edge.
func (_q *GroupQuery) QueryPowers() *PowerQuery {
...
func (_q *GroupQuery) QueryGroupPowers() *GroupPowerQuery {
...
func (_q *GroupQuery) Clone() *GroupQuery {
	...
	return &GroupQuery{
		...
		withPowers:      _q.withPowers.Clone(),
...
// WithPowers tells the query-builder to eager-load the nodes that are connected to
// the "powers" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *GroupQuery) WithPowers(opts ...func(*PowerQuery)) *GroupQuery {
	query := (&PowerClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *GroupQuery) WithGroupPowers(opts ...func(*GroupPowerQuery)) *GroupQuery {
	query := (&GroupPowerClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := _q.withPowers; query != nil {
		if err := _q.loadPowers(ctx, query, nodes,
			func(n *Group) { n.Edges.Powers = []*Power{} },
			func(n *Group, e *Power) { n.Edges.Powers = append(n.Edges.Powers, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withGroupPowers; query != nil {
		if err := _q.loadGroupPowers(ctx, query, nodes,
			func(n *Group) { n.Edges.GroupPowers = []*GroupPower{} },
			func(n *Group, e *GroupPower) { n.Edges.GroupPowers = append(n.Edges.GroupPowers, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *GroupQuery) loadPowers(ctx context.Context, query *PowerQuery, nodes []*Group, init func(*Group), assign func(*Group, *Power)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(group.PowersTable)
		s.Join(joinT).On(s.C(power.FieldID), joinT.C(group.PowersPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(group.PowersPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(group.PowersPrimaryKey[0]))
	...
	neighbors, err := withInterceptors[[]*Power](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "powers" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (_q *GroupQuery) loadGroupPowers(ctx context.Context, query *GroupPowerQuery, nodes []*Group, init func(*Group), assign func(*Group, *GroupPower)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Group)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(grouppower.FieldGroupID)
	}
	query.Where(predicate.GroupPower(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(group.GroupPowersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
