...
type S3Bucket struct {
	...
	// VirtualPathID holds the value of the "virtual_path_id" field.
	VirtualPathID int `json:"virtual_path_id,omitempty"`
...
func (*S3Bucket) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case s3bucket.FieldID, s3bucket.FieldVirtualPathID:
			values[i] = new(sql.NullInt64)
		case s3bucket.FieldBucketName, s3bucket.FieldUserID, s3bucket.FieldRegion, s3bucket.FieldVersioning:
	...
	for i := range columns {
		...
		case s3bucket.FieldVirtualPathID:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field virtual_path_id", values[i])
			} else if value.Valid {
				_m.VirtualPathID = int(value.Int64)
			}
		case s3bucket.FieldVersioning:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field versioning", values[i])
			} else if value.Valid {
				_m.Versioning = value.String
			}
...
func (_m *S3Bucket) Unwrap() *S3Bucket {
	...
	if !ok {
		panic("gen: S3Bucket is not a transactional entity")
	}
...
func (_m *S3Bucket) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("virtual_path_id=")
	builder.WriteString(fmt.Sprintf("%v", _m.VirtualPathID))
	builder.WriteString(", ")
	builder.WriteString("versioning=")
	builder.WriteString(_m.Versioning)
	builder.WriteString(", ")
