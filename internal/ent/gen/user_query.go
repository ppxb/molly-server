...
// QueryUserFiles chains the current query on the "user_files" edge.
func (_q *UserQuery) QueryUserFiles() *UserFileQuery {
	query := (&UserFileClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withUserFiles: _q.withUserFiles.Clone(),
...
// WithUserFiles tells the query-builder to eager-load the nodes that are connected to
// the "user_files" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithUserFiles(opts ...func(*UserFileQuery)) *UserQuery {
	query := (&UserFileClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withUserFiles = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withUserFiles; query != nil {
		if err := _q.loadUserFiles(ctx, query, nodes,
			func(n *User) { n.Edges.UserFiles = []*UserFile{} },
			func(n *User, e *UserFile) { n.Edges.UserFiles = append(n.Edges.UserFiles, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadUserFiles(ctx context.Context, query *UserFileQuery, nodes []*User, init func(*User), assign func(*User, *UserFile)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(userfile.FieldUserID)
	}
	query.Where(predicate.UserFile(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.UserFilesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
